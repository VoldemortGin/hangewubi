...
if let cls = NSClassFromString("InputController") {
    NSLog("[函戈五笔] 找到 InputController 类: \(cls)")
} else {
    NSLog("[函戈五笔] ⚠️ 无法找到 InputController 类！尝试模块限定名...")
    if let cls = NSClassFromString("HangeWubi.InputController") {
        NSLog("[函戈五笔] 找到 HangeWubi.InputController: \(cls)")
    } else {
        NSLog("[函戈五笔] ⚠️ 也无法找到 HangeWubi.InputController")
    }
}
